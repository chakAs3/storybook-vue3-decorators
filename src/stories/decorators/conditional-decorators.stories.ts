...
const VueConditionalWrapperWrapper = (storyFn, context) => {
  // Call the `storyFn` to receive a component that Vue can render
  const story = storyFn();
  const animated = context.args.animate
  console.log({animated});
  // Vue 3 "Functional" component as decorator
  return () => context.args.show ? h('div', { style: 'border: 10px solid black;padding: 20px' }, h(story, {...context.args })):h(story,{ label: 'Disconnected',backgroundColor: 'gray',animate:false});
};
...
export const AnimateConditionalDecorator = {
